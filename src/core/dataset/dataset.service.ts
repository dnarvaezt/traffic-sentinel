// ─── Dataframe-based query engine (Danfo.js) ─────────────────────────────────
...
export function executeQuery(
  ...
  if (config.filters?.length) df = filterDataFrame(df, config.filters)
  if (config.metrics?.length) df = aggregateDataFrame(df, config.metrics, config.groupBy)
  if (config.sorts?.length) df = sortDataFrame(df, config.sorts)
